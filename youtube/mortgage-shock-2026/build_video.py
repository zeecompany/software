...
# --- Per-chapter on-screen graphics ---------------------------------------
# headline: short 2-line title card; stat/stat_label: the big number card;
# bullets: optional list (used for the "5 things to do" chapter).
GRAPHICS = {
    "s1_hook": dict(
        headline=("UK BORROWING COSTS", "HIT 28-YEAR HIGH"),
        stat="5.89%", stat_label="30-YEAR GILT YIELD · HIGHEST SINCE 1998",
        tag="BREAKING", accent=RED,
    ),
    "s2_numbers": dict(
        headline=("THE NUMBERS", "BEHIND THE SHOCK"),
        stat="4.52%", stat_label="5-YEAR SWAP RATE · 3-YEAR HIGH · PRICES YOUR MORTGAGE",
        tag="THE NUMBERS", accent=YELLOW,
        rows=[("30-yr gilt", "5.89%", "highest since 1998"),
              ("10-yr gilt", "5.20%+", "highest since 2008"),
              ("5-yr swap", "4.52%", "highest since Oct 2023")],
    ),
    "s3_why": dict(
        headline=("WHY IS THIS", "HAPPENING?"),
        stat="$94", stat_label="OIL PER BARREL · UK CPI 2.9% AND RISING",
        tag="THE CAUSE", accent=YELLOW,
        rows=[("US–Iran fire exchanged", "Oil +3.8%", ""),
              ("UK inflation (July)", "2.9%", "BoE sees ~3.2% by Q4"),
              ("Bond investors", "SELL", "yields rise, mortgages follow")],
    ),
    "s4_cliff": dict(
        headline=("THE 1.8 MILLION", "MORTGAGE CLIFF"),
        stat="1.8M", stat_label="FIXED-RATE DEALS ENDING IN 2026 · SOURCE: UK FINANCE",
        tag="WHO IS HIT", accent=RED,
        rows=[("5-yr fix, Sept 2021", "2.75%", "many below 2%"),
              ("5-yr fix, today", "5.63%", "Moneyfacts average"),
              ("2-yr fix, today", "5.59%", "next BoE decision: 17 Sept")],
    ),
    "s5_math": dict(
        headline=("YOUR PAYMENT", "SHOCK"),
        stat="+£321", stat_label="PER MONTH · £200K MORTGAGE · 2.75% TO 5.63%",
        tag="THE MATHS", accent=RED,
        rows=[("2021 deal @ 2.75%", "£923 / month", ""),
              ("Re-fix @ 5.63%", "£1,244 / month", ""),
              ("Extra cost", "+£3,852 / year", "before any further rises")],
    ),
    "s6_budget": dict(
        headline=("THE 28 OCTOBER", "BUDGET"),
        stat="£25bn", stat_label="POSSIBLE TAX RISES · CAPITAL ECONOMICS ESTIMATE",
        tag="TAX", accent=YELLOW,
        rows=[("Fiscal headroom", "£26bn to £14bn", "after the bond rout"),
              ("Off the table", "Income tax · VAT · NI rates", "manifesto pledge"),
              ("In play", "CGT · pensions · IHT · levy", "+ frozen thresholds")],
    ),
    "s7_actions": dict(
        headline=("5 THINGS TO DO", "RIGHT NOW"),
        stat="5", stat_label="ACTIONS BEFORE YOUR FIX ENDS",
        tag="ACTION PLAN", accent=(46, 204, 113),
        bullets=["Know the exact date your fixed deal ends",
                 "Lock a new rate up to 6 months early — switch if cheaper later",
                 "Never drift onto the Standard Variable Rate",
                 "Overpay while your rate is cheap (usually 10%/yr penalty-free)",
                 "Use ISA & pension allowances before the Budget"],
    ),
    "s8_outro": dict(
        headline=("WHAT RATE ARE", "YOU COMING OFF?"),
        stat="17 SEPT", stat_label="BANK OF ENGLAND DECISION · BUDGET 28 OCT",
        tag="YOUR TURN", accent=YELLOW,
    ),
}
...
def render_overlay(seg_id: str, chapter: str, idx: int, total: int, W: int, H: int) -> Path:
    ...
    if "bullets" in g:
        ...
        for i, b in enumerate(g["bullets"]):
            y0 = by + i * int(78 * s)
            d.rounded_rectangle((px, y0, px + bw, y0 + int(64 * s)), radius=int(14 * s), fill=(10, 10, 14, 200))
            d.ellipse((px + int(14 * s), y0 + int(12 * s), px + int(54 * s), y0 + int(52 * s)), fill=accent)
            d.text((px + int(34 * s), y0 + int(32 * s)), str(i + 1), font=font("Roboto-Black.ttf", int(28 * s)), fill=BLACK, anchor="mm")
            # auto-fit bullet text
            fs_b = int(30 * s); fb = font("Roboto-Bold.ttf", fs_b)
            while text_size(d, b, fb)[0] > bw - int(90 * s) and fs_b > int(18 * s):
                fs_b -= 1; fb = font("Roboto-Bold.ttf", fs_b)
            d.text((px + int(72 * s), y0 + int(32 * s)), b, font=fb, fill=WHITE, anchor="lm")
    ...
    src = "Sources: Guardian · BBC · Telegraph · UK Finance · ONS · Moneyfacts · Resolution Foundation"
...
def render_title_card(W: int, H: int) -> Path:
    ...
    img = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    d = ImageDraw.Draw(img)
    d.text((W // 2, int(H * 0.36)), "MORTGAGE SHOCK", font=font("Roboto-Black.ttf", int(170 * s)),
           fill=YELLOW, anchor="mm", stroke_width=int(10 * s), stroke_fill=BLACK)
    d.text((W // 2, int(H * 0.53)), "UK BORROWING COSTS HIT A 28-YEAR HIGH", font=font("Roboto-Black.ttf", int(60 * s)),
    ...
    d.text((W // 2, int(H * 0.63) + int(37 * s)), "WHAT IT MEANS FOR YOUR MONEY", font=font("Roboto-Black.ttf", int(34 * s)),
           fill=WHITE, anchor="mm")
    d.text((W // 2, int(H * 0.80)), "3 SEPTEMBER 2026", font=font("Roboto-Bold.ttf", int(34 * s)),
...
def render_end_card(W: int, H: int) -> Path:
    ...
    d.text((int(96 * s), int(225 * s)), "WATCHING", font=font("Roboto-Black.ttf", int(96 * s)), fill=YELLOW,
           stroke_width=int(6 * s), stroke_fill=BLACK)
    lines = ["Subscribe for the Bank of England decision (17 Sept)",
             "and full Budget breakdown (28 Oct).",
             "Comment: what rate are you coming off?"]
...
def render_chapter(seg: dict, idx: int, total: int, W: int, H: int, preset: str, crf: int,
    ...
    seg_id = seg["id"]
    audio = ASSETS / "audio" / f"{seg_id}.mp3"
...
def main() -> None:
    ...
    durs = [probe_duration(ASSETS / "audio" / f"{s['id']}.mp3") + TAIL_PAD for s in segs]
    total_len = INTRO_LEN + sum(durs) + OUTRO_LEN
    print(f"Planned length: {total_len:.1f}s ({total_len / 60:.2f} min) at {W}x{H}")

    clips: list[Clip] = []
    clips.append(render_card("intro", render_title_card(W, H), ASSETS / "images" / "bg_city.jpg",
    ...
    clips.append(render_card("end", render_end_card(W, H), ASSETS / "images" / "bg_sunrise.jpg",
