#!/usr/bin/env python3
"""
Builds the YouTube thumbnail (1280x720) for the Budget Tax Raid video.
...
def build():
    ...
    bg = Image.open(ROOT / "assets/images/bg_westminster.jpg").convert("RGB")
    ...
    fw, fh = face.size
    crop = face.crop((0, 0, int(fw * 0.58), fh))
    target_h = H
    ...
    draw_outlined(d, (tx, 78), "£25bn TAX", f_big, YELLOW, stroke=10)
    draw_outlined(d, (tx, 205), "RAID", f_big, WHITE, stroke=10)

    # £321 / MONTH red pill
    pill = (tx - 6, 352, tx + 560, 470)
    d.rounded_rectangle(pill, radius=22, fill=RED, outline=BLACK, width=6)
    draw_outlined(d, (tx + 22, 362), "7 TAXES", font("Roboto-Black.ttf", 92), WHITE, stroke=6)
    draw_outlined(d, (tx + 400, 386), "RISING", font("Roboto-Black.ttf", 40), WHITE, stroke=5)

    # Sub-line
    draw_outlined(d, (tx, 500), "YOUR SAVINGS · PENSION · HOME", font("Roboto-Black.ttf", 40), YELLOW, stroke=6)
    ...
    bd.text((210, 40), "28 OCTOBER", font=font("Roboto-Black.ttf", 46), fill=BLACK, anchor="mm")
    bd.text((210, 92), "BUDGET DAY", font=font("Roboto-Bold.ttf", 26), fill=BLACK, anchor="mm")
    ...
    d.text((160, H - 64), "WARNING", font=font("Roboto-Black.ttf", 40), fill=WHITE, anchor="mm")
